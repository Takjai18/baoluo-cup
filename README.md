...
1. **預先登記姓名** — 賽前輸入 16 人姓名，用 checkbox 二選一所屬教會（陀螺可留空）
2. **當日登記陀螺** — 報到時按「登記陀螺」，為每人登記 **3 隻** 配置：
   - **上蓋（Blade）**：輸入 `BX49` / `UX15` / `CX07` / `CX15` 即可
   - **CX 組裝**：鎖定紋章 + 主刃／金屬戰刃 + 輔助戰刃（代碼）；**Expand（CX13+）** 必須再選超越戰刃
   - **固鎖（Ratchet）**：完整下拉（0-60…M-85）+ 常用快速勾選
   - **軸心（Bit）**：完整代碼下拉（H、LR、J、LO…）+ 常用快速勾選
   - 顯示例：`BX49 3-60 J`、`蒼龍 勇氣 J 3-60 H`、`龍王 閃擊 B K 3-60 H`
3. 系統會提示本活動 T0/T1 與零件重複限制（仍可強制儲存）
4. 未齊 3 隻也可開始比賽（會警告），之後仍可補登

## 操作流程（當日）

1. **選手** — 預登姓名（若未做）→ 當日登記 3 隻陀螺 →「開始比賽 · 產生第 1 輪」
2. **對戰表** — 公佈 8 場 → 每場完結按「輸入結果」（選勝方 + 雙方 Battle Points）
3. 8 場齊 →「鎖定本輪 · 進入下一輪」（自動按規則配對）
4. 第 4 輪鎖定後 → **排名** 看前 4 → **淘汰賽** 產生準決賽
5. 賽後用 **匯出** 下載 CSV（含陀螺）／文字報告／JSON 備份
